...
# Define instruction types - defined without prefixes (the instructions_ prefix will be added in the file path)
...
run_file_extraction() {
    ...
    mkdir -p "$output_dir/file_lists/o4-mini"
...
generate_file_lists() {
    ...
    # Create benchmark/file_lists/o4-mini directory if it doesn't exist
    mkdir -p "./benchmark/file_lists/o4-mini"
    ...
    for instruction_type in "${instruction_types[@]}"; do
        ...
        for i in {1..3}; do
            ...
            for project in "${PROJECTS[@]}"; do
                # 테스트 파일 생성 (실행 번호 포함)
                test_file="./benchmark/file_lists/o4-mini/test-${project}-${instruction_type}_${i}.json"
                
                # Find file - also handle project names with hyphens
                # First try with exact name
                generated_file="./benchmark/file_lists/o4-mini/${project}-${instruction_type}.json"
                
                # If file not found, search all JSON files in directory
                if [ ! -f "$generated_file" ]; then
                    # Find files that start with project name (handle projects with hyphens)
                    found_file=$(find ./benchmark/file_lists/o4-mini/ -name "${project}*-${instruction_type}.json" -type f | head -n 1)
...
RETRY_DELAY=60  # Waiting time between retries (seconds)
BETWEEN_RUNS_DELAY=120  # Waiting time between benchmark runs (seconds)
BETWEEN_PROJECTS_DELAY=300  # Waiting time between projects (seconds)
...
for instruction_type in "${INSTRUCTION_TYPES[@]}"; do
    ...
    echo "Instructions will be loaded from: .docs/instructions_${instr_type}.md file"
    ...
    for run_number in {1..3}; do
        ...
        for project in "${PROJECTS[@]}"; do
            ...
            test_file="./benchmark/file_lists/o4-mini/test-${project}-${instr_type}_${run_number}.json"
            ...
            while [ $retry_count -lt $MAX_RETRIES ] && [ "$success" = false ]; do
                ...
                if [ -z "$BENCHMARK_DIR" ]; then
                    ...
                    # For remaining projects: use saved directory name
